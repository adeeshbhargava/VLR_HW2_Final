...
def build_transforms():
    ...
    ds_transforms =  ds_transforms = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))])
...
def get_optimizers_and_schedulers(gen, disc):
    ...
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator, lambda i: 1 - min(i, 500_000) / 500_000)

    optim_generator = torch.optim.Adam(gen.parameters(), lr=0.0002, betas=(0, 0.9))
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(optim_generator, lambda i: 1 - min(i, 100_000) / 100_000)   
    
    #Anvesh definitions
    optim_discriminator = torch.optim.Adam(disc.parameters(), 0.0002, betas=(0, 0.9))
    scheduler_discriminator = torch.optim.lr_scheduler.StepLR(optim_discriminator, 0.01, 1e5)
    
    optim_generator = torch.optim.Adam(gen.parameters(), 0.0002, betas=(0, 0.9))
    scheduler_generator = torch.optim.lr_scheduler.StepLR(optim_generator, 0.01, 2e4) 
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # TODO 1.3: compute the discriminator loss
                ...
                interp = (eps * train_batch + ((1 - eps) * gen_out)).requires_grad_(True)
